...
from networks.hmrnet import HMR, Bottleneck
...
net = HMR(Bottleneck, [3, 4, 6, 3], mean_params.to(device))
checkpoint = torch.load(conf['networks']['checkpoint_weight'])
net.load_state_dict(checkpoint['recon_net'])
net.eval().to(device)
...
img = get_demo_input_using_keyp_RCNN('examples/test3.png', rcnn_model, device)
# img = get_demo_input('examples/test.jpg')
with torch.no_grad():
    pred_param = net(img.to(device), None, None, False)
