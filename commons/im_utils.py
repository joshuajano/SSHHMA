...
import cv2
import random
def augm_color(img, pn):
    img = np.clip(img * pn[None,None,:], 0, 255)
    return img
def rgb_preprocessing(img, center, scale, rot, flip, pn, IMG_RES = 256):
    #color augmentation
    img = augm_color(img, pn)
    img = img.astype(np.float32)
    img = crop(img, center, scale, [IMG_RES, IMG_RES], rot=rot)
